...
def get_config_from_file(config_file: str, args_config: Namespace) -> Config:
    ...
    image_format = get_param_or_default(
        config, "image_format", str(default_config.image_format).split(".")[-1], args_config.image_format
    )
    image_format = match_image_format[image_format.upper()]
    image_quality = int(
    ...
    output_format = get_param_or_default(
        config, "output_format", str(default_config.output_format).split(".")[-1], args_config.output_format
    )
    output_format = match_output_format[output_format.upper()]
    pause_sec = int(get_param_or_default(config, "pause", default_config.pause_sec, args_config.pause))
